...
TEST(tool_registry_definitions_count) {
    ...
    // 4 read-only tools + find_symbol + present_plan + print + signal_completion + run_shell + spawn_agent
...
TEST(tool_registry_act_mode_has_write_tools) {
    ...
    auto defs = registry.definitions();
    // Should have 14 tools: 4 read-only + find_symbol + print + signal_completion + run_shell + spawn_agent + 5 write (write_file, edit_file, create_dir, delete_file, delete_dir)
